...
public class OptionsDialog extends Dialog implements ActionListener, ItemListener {
	...
	JPanel FormatPanel, MiscPanel, DefaultsPanel, SecPanel;
	...
	public OptionsDialog(OptionsDialogCallback cb_) {
		...
		fullColour = addRadioCheckbox("Hight (all available colours)", colourGroup, colourPanel);
...
//		addGBComponent(menuKey, InputsPanel, 1, 3, 1, GridBagConstraints.REMAINDER, 2, 2, 25, 1, GridBagConstraints.NONE, GridBagConstraints.FIRST_LINE_START, new Insets(4, 5, 0, 5));
		...
		Object[] scalingFactors = { "Auto", "Fixed Aspect Ratio", "50%", "75%", "95%", "100%", "105%", "125%", "150%", "175%", "200%", "250%", "300%", "350%", "400%" };
...
//		addGBComponent(defaultsPanel, DefaultsPanel, 1, 0, 1, GridBagConstraints.REMAINDER, 0, 0, 1, 1, GridBagConstraints.HORIZONTAL, GridBagConstraints.PAGE_START, new Insets(4, 0, 4, 5));
		...
		encX509 = addJCheckBox("TLS with X.509 certificates", null, encryptionPanel, new GridBagConstraints(0, 2, 1, 1, 1, 1, GridBagConstraints.LINE_START, GridBagConstraints.REMAINDER, new Insets(
				0, 0, 0, 60), 0, 0));
...
//					break;
	...
	JCheckBox addJCheckBox(String str, ButtonGroup group, JPanel panel, GridBagConstraints gbc) {
		JCheckBox c = new JCheckBox(str);
		panel.add(c, gbc);
		if (group != null)
			group.add(c);
	...
	public void endDialog() {
		super.endDialog();
		CConn cc = (CConn) cb;
		if (cc.getViewport() != null && cc.getViewport().isVisible()) {
			cc.getViewport().requestFocus();
		}
	}
...
//			veryLowColour.setEnabled(!autoSelect.isSelected());
		...
		if (s instanceof JCheckBox && (JCheckBox) s == encNone) {
			secNone.setSelected(encNone.isSelected() && UserPreferences.getBool("viewer", "secNone", true));
			secVnc.setSelected(encNone.isSelected() && UserPreferences.getBool("viewer", "secVnc", true));
		}
