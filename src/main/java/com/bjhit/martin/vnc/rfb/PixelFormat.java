...
public class PixelFormat {
	...
	public boolean equal(PixelFormat x) {
		return (bpp == x.bpp && depth == x.depth && (bigEndian == x.bigEndian || bpp == 8) && trueColour == x.trueColour && (!trueColour || (redMax == x.redMax && greenMax == x.greenMax
				&& blueMax == x.blueMax && redShift == x.redShift && greenShift == x.greenShift && blueShift == x.blueShift)));
	}

	public void read(BaseInputStream is) {
		bpp = is.readU8();
		depth = is.readU8();
	...
	// os.pad(3);
	// }
	public byte[] getPixelFormatByte() {
		byte[] buff = new byte[16];
		buff[0] = (byte) (bpp & 0xFF);
		buff[1] = (byte) (depth & 0xFF);
		buff[2] = (byte) (bigEndian ? 1 : 0 & 0xFF);
		buff[3] = (byte) (trueColour ? 1 : 0 & 0xFF);
		buff[4] = (byte) (redMax >> 8 & 0xFF);
		buff[5] = (byte) (redMax & 0xFF);
		buff[6] = (byte) (greenMax >> 8 & 0xFF);
		buff[7] = (byte) (greenMax & 0xFF);
		buff[8] = (byte) (blueMax >> 8 & 0xFF);
		buff[9] = (byte) (blueMax & 0xFF);
		buff[10] = (byte) (redShift & 0xFF);
		buff[11] = (byte) (greenShift & 0xFF);
		buff[12] = (byte) (blueShift & 0xFF);
		return buff;
	}
	...
	public String toString() {
		...
		return "[" + bpp + " " + depth + " " + depth + " " + bigEndian + " " + trueColour + " " + redMax + " " + greenMax + " " + blueMax + " " + redShift + " " + greenShift + " " + blueShift + "]";
	}
	...
	public String print() {
		...
		if (blueShift == 0 && greenShift > blueShift && redShift > greenShift && blueMax == (1 << greenShift) - 1 && greenMax == (1 << (redShift - greenShift)) - 1
				&& redMax == (1 << (depth - redShift)) - 1) {
			s.append(" rgb" + (depth - redShift) + (redShift - greenShift) + greenShift);
			return s.toString();
		}

		if (redShift == 0 && greenShift > redShift && blueShift > greenShift && redMax == (1 << greenShift) - 1 && greenMax == (1 << (blueShift - greenShift)) - 1
				&& blueMax == (1 << (depth - blueShift)) - 1) {
			s.append(" bgr" + (depth - blueShift) + (blueShift - greenShift) + greenShift);
			return s.toString();
		}

		s.append(" rgb max " + redMax + "," + greenMax + "," + blueMax + " shift " + redShift + "," + greenShift + "," + blueShift);
		return s.toString();
	}
	...
	public static final PixelFormat verylowColourPF = new PixelFormat(8, 3, false, true, 1, 1, 1, 2, 1, 0);
	public static final PixelFormat lowColourPF = new PixelFormat(8, 6, false, true, 3, 3, 3, 4, 2, 0);
	public static final PixelFormat mediumColourPF = new PixelFormat(8, 8, false, false, 7, 7, 3, 0, 3, 6);
}
