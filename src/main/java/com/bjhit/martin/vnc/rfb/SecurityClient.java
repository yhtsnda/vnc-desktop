...
public class SecurityClient extends Security {
	...
	public CSecurity GetCSecurity(int secType) {
		...
		if (!IsSupported(secType))
			throw new RuntimeException("Security type not supported");
...
//			return (new CSecurityStack(secTypeTLSNone, "TLS with no password", new CSecurityTLS(true), null));
//		case Security.secTypeTLSVnc:
//			return (new CSecurityStack(secTypeTLSVnc, "TLS with VNCAuth", new CSecurityTLS(true), new CSecurityVncAuth()));
//		case Security.secTypeTLSPlain:
//			return (new CSecurityStack(secTypeTLSPlain, "TLS with Username/Password", new CSecurityTLS(true), new CSecurityPlain()));
//		case Security.secTypeTLSIdent:
//			return (new CSecurityStack(secTypeTLSIdent, "TLS with username only", new CSecurityTLS(true), new CSecurityIdent()));
//		case Security.secTypeX509None:
//			return (new CSecurityStack(secTypeX509None, "X509 with no password", new CSecurityTLS(false), null));
...
//			return (new CSecurityStack(secTypeX509Plain, "X509 with Username/Password", new CSecurityTLS(false), new CSecurityPlain()));
//		case Security.secTypeX509Ident:
//			return (new CSecurityStack(secTypeX509Ident, "X509 with username only", new CSecurityTLS(false), new CSecurityIdent()));
		default:
			throw new RuntimeException("Security type not supported");
		}
...
//	}
	...
	static StringParameter secTypes = new StringParameter("SecurityTypes", "Specify which security scheme to use (None, VncAuth)",
