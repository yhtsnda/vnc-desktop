...
public class ConnParams {
	...
	public boolean readVersion(BaseInputStream is) {
		done = false;
		if (verStrPos >= 12)
			return false;
	...
	public void writeVersion(OutStream os) {
		...
		os.writeBuffer(str.getBytes(), 0, 12);
	}
	...
	public void setPF(PixelFormat pf) {
		...
		if (pf.bpp != 8 && pf.bpp != 16 && pf.bpp != 32) {
			throw new PixelFormatException("setPF: not 8, 16 or 32 bpp?");
		}
	}
	...
	public void setEncodings(int nEncodings, int[] encodings) {
		...
		for (int i = nEncodings - 1; i >= 0; i--) {
			...
			else if (encodings[i] >= Encodings.pseudoEncodingCompressLevel0 && encodings[i] <= Encodings.pseudoEncodingCompressLevel9) {
				customCompressLevel = true;
				compressLevel = encodings[i] - Encodings.pseudoEncodingCompressLevel0;
			} else if (encodings[i] >= Encodings.pseudoEncodingQualityLevel0 && encodings[i] <= Encodings.pseudoEncodingQualityLevel9) {
				noJpeg = false;
				qualityLevel = encodings[i] - Encodings.pseudoEncodingQualityLevel0;
			} else if (encodings[i] <= Encodings.encodingMax && Encoder.supported(encodings[i]))
		...
		for (int i = nEncodings - 1; i >= 0; i--) {
			if (encodings[i] >= Encodings.pseudoEncodingFineQualityLevel0 + 1 && encodings[i] <= Encodings.pseudoEncodingFineQualityLevel100) {
				noJpeg = false;
				fineQualityLevel = encodings[i] - Encodings.pseudoEncodingFineQualityLevel0;
			} else if (encodings[i] >= Encodings.pseudoEncodingSubsamp1X && encodings[i] <= Encodings.pseudoEncodingSubsampGray) {
				noJpeg = false;
